...
| 工具 | 说明 |
|------|------|
| **翻译** | 智能路由 + 双引擎完全离线：短词和术语走词典，整句自动走本地神经模型；长文本按句分片，代码标签、变量名、字段名和自定义术语会先保护再还原。7.7 万中文词条，中英双向，首次加载模型约 10-30 秒，支持公司内网翻译接口 |
...
如果插件内置模型仍不能满足质量要求，可以把翻译放到内网离线服务，插件继续作为轻量客户端使用：

```text
Chrome 插件 → 公司内网 /translate → CTranslate2 + M2M100-418M int8
```

服务代码位于 [`server/translation-service`](server/translation-service/README.md)，单个 M2M100 模型支持中英双向、CPU int8 推理、术语保护和代码符号保护。运行期间无需公网。

## 📌 近期更新

- **v1.7.0** 智能翻译默认路由；长文本自动分片；代码标签、变量名、字段名、URL 和自定义术语保护；修复长段落静默截断与代码标识符被破坏的问题。
- **v1.6.0** 按代码评审优化：神经翻译增加「模型未下载」可发现引导 + Offscreen Document 常驻加载（避免弹窗关闭中断）；内网接口支持「仅本次会话」避免密钥明文落盘；词典速查建首字索引提速；内容脚本大词典改为按需加载；词典锁定策略注释 + 反查回归测试；下载脚本增加错误页检测与 sha256 基线。
- **v1.5.0** 引入本地神经整句翻译（Transformers.js + opus-mt-zh-en，完全离线，约 110MB 模型）。
...
翻译工具提供**智能翻译**与**词典翻译**两种入口：

- **智能翻译**（推荐）：短词/术语走词典，整句自动走本地神经模型；长文本按句分片，代码标签、变量名、字段名、URL 和自定义术语会先保护再还原
- **词典翻译**：基于本地词典逐词直译，适合查术语和快速参考；结构词（的/了/把等）自动省略
- **神经引擎（中英双向）**：本地 WASM 运行 [Transformers.js](https://github.com/huggingface/transformers.js) +
  opus-mt 系列模型（Marian NMT）。中→英用 `opus-mt-zh-en`、英→中用 `opus-mt-en-zh`，
  按输入是否含中文自动判定方向。两个模型文件随 zip 分发（共约 220MB），不发任何网络请求。
  首次点击需加载模型约 10-30 秒
...
└── server/translation-service/ # 可选：内网离线 M2M100 翻译服务
```
