...
## v1.7.0 修复记录（2026-09-18）

本版本针对 1.6.0 的翻译准确率和其他工具缺陷完成了以下修复：

- 增加“智能翻译”入口：短术语走词典，整句自动走神经模型；词典翻译保留为次级入口。
- 神经翻译增加长文本分片，避免超过模型上限后静默截断。
- 神经翻译增加代码标签、变量名、字段名、URL、文件路径保护，翻译后原样还原。
- 自定义词库和常见前端术语参与神经翻译，优先输出稳定术语。
- 修正 Offscreen Document 的 `documentUrl` 参数和 `WORKERS` reason。
- 主机权限改为用户点击后首先申请，避免异步等待导致 user-gesture 失效。
- 修复 JSON→YAML 对冒号、井号等字符串的错误输出。
- CSV/TSV 改为完整解析引号字段和引号内换行。
- Cron 的 `1-5` 正确描述为“周一至周五”。
- 图片工具输出完整 CSS/HTML Data URI，不再使用 60 字符截断占位。
- HTML 还原改用 `DOMParser`，避免 `innerHTML` 解析副作用。
- URL 参数 JSON 保留重复 key 的数组值。
- 随机密码保证包含已选择的字符类型，并改用无偏随机索引。
- 冒烟测试扩展到 48 条，并完成浏览器端核心功能回归。
